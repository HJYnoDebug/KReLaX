...
# File paths
BEST_MODEL_PATH = 'best_model/xlm_r_best_model.pt'
CHECKPOINT_PATH = 'checkpoint/xlm_r_checkpoint.pt'
TRAIN_DATA_PATH = 'checkpoint/xlm_r_train_data.txt'
...
model = XLMRobertaForSequenceClassification.from_pretrained('xlm-roberta-base', num_labels=label_num).to(device)
...
for epoch in range(hyperparams.num_epochs):
    ...
    for batch in train_dataloader:
        ...
        # Create mask to ignore gradients for labels == -1
        mask = (labels != -1).float()
        ...
        loss = criterion(logits * mask, labels * mask)  # Mask logits and labels
        loss.backward()
        optimizer.step()
    ...
    with torch.no_grad():
        for batch in test_dataloader:
            ...
            # Ignore labels equal to -1
            mask = (labels != -1).float()
            loss = (loss * mask).sum() / mask.sum()  # Calculate loss only for valid labels
    ...
    for i, label_name in enumerate(label_names):
        # Get true and predicted labels for each label
        true_labels = np.array(all_labels)[:, i]
        pred_labels = binary_predictions[:, i]
        
        # Mask to exclude -1 labels
        mask = true_labels != -1
    
        # Calculate metrics for valid labels
